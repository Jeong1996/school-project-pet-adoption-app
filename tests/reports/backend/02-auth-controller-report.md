...
Control Flow Testing was selected for auth controller because it manages multiple HTTP response paths based on different conditions. For errors under different scenarios, it must handle correctly and return the corresponding status code. For validation errors, it should return 400 Bad Request. For service errors, the appropriate 400 or 401 status codes need to be sent out. Meanwhile, 201 Created or 200 OK should be returned with successful operations. Each of these branches represents a different flow through the code and needs to work properly for the API to function. By testing each branch explicitly, we ensure that the controller maps all possible scenarios to their expected HTTP responses correctly. 

In our system, the controller acts as a data pipeline between HTTP requests and backend services, and thus Data Flow Testing was applied here. The data flows from the request body is being extracted and validated. Then it will be passed to service methods for processing and finally transformed into HTTP responses. This transformation pipeline must be able to handle both successful results and various error conditions. Testing the data flow ensures that all information, including Email, password, name, and role are correctly passed from the request through the service layer and then transformed properly in response. 
